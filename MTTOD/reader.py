...
class BaseIterator(object):
    ...
    def get_data_iterator(self, all_batches, task, ururu, simplify=False,add_auxiliary_task=False, context_size=-1):
    ...
    def get_data_iterator(self, all_batches, task, ururu, simplify=False,add_auxiliary_task=False, context_size=-1):
        for dial_batch in all_batches:
            ...
            for dial in dial_batch:
                ...
                dial_history = []
                dial_history_simplified = []
                span_history = []
                for turn in dial:
                    context, span_dict = self.flatten_dial_history(
                        dial_history, span_history, len(turn["user"]), context_size)
                    if simplify:
                        encoder_input_ids = dial_history_simplified + turn["user"] + [self.reader.eos_token_id]
                    else:
                    ...
                    span_label_tokens = ["O"] * len(context + turn["user"] + [self.reader.eos_token_id])
                    ...
                    if ururu:
                        ...
                        if task == "dst":
                            ...
                            turn_text_simplified=bspn+ turn["resp"]  
                        else:
                            turn_text = turn["user"] + bspn + \
                                turn["dbpn"] + turn["aspn"] + turn["redx"]
                            turn_text_simplified=bspn+turn["redx"]
                    dial_history_simplified=turn_text_simplified
...
class MultiWOZReader(BaseReader):
    ...
    def encode_data(self, data_type):
        ...
        data= self.fix_dialog_state(data)
        encoded_data = []
    ...
    def fix_dialog_state(self, data):
        for dial_id in data:
            ...
            for turn_id, turn in enumerate(dial):
                ...
                if 'name' in cons:
                    cons_dict=self.bspn_to_constraint_dict(cons)
                    ...
                    turn['constraint']=self.constraint_dict_to_bspn(cons_dict)
        return data
