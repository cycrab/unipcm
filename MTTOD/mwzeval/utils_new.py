...
def load_multiwoz22():
    ...
    with open("mwzeval/data/references/dialog_acts.json") as f:
        print("Downloading MultiWOZ_2.2/dialog_act.json ")
        dialog_acts = json.loads(f.read())#.decode()
    ...
    for f, n in folds.items():
        for i in range(n):
            ...
            with open(f"mwzeval/data/references/{f}/dialogues_{str(i+1).zfill(3)}.json") as file:
                raw_data.extend(json.loads(file.read()))
