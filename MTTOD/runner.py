...
class MultiWOZRunner(BaseRunner):
    ...
    def train(self):
        ...
        if self.cfg.few_shot:
            train_num = int(len(train_batches)/10)
            train_batches = train_batches[:train_num]
            self.cfg.epochs = 25
    ...
    def predict(self):
        ...
        for dial_batch in tqdm(pred_batches, total=len(pred_batches), desc="Prediction"):
                ...
                if self.cfg.task == "e2e":
                    ...
                    if self.cfg.use_true_curr_aspn:
                        for t, _dbpn in enumerate(dbpn):
                            ...
                            with torch.no_grad():#max_length = 300
            ...
            for dial_id,dial in result.items():
                ...
                simplified_dial=[]
                for turn in dial:
                    simplified_turn=turn['turn_domain'][0]
        ...
        # save is added after processing
        ...
        for id,dial in results.items():
            ...
            for turn in dial:
                temp={}
                turn['resp_gen']=turn['resp_gen'].replace(' <eos_resp>','')
                temp['response']=turn['resp_gen'].replace('<bos_resp> ','')
                temp['state']=self.reader.bspn_to_constraint_dict(turn['bspn_gen'])
                input_data[id].append(temp)
        if self.cfg.task == "e2e":
            #e = MultiWozEvaluator()
