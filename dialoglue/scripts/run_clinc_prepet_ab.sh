...
CUDA_VISIBLE_DEVICES=5 python dialoglue/run.py \
...
--prepet \
--output_dir dialoglue/checkpoints/clinc_10_prepet_ab/ \
--train_batch_size 64 --dropout 0.1 --num_epochs 10 --learning_rate 3e-5 \
--model_name_or_path checkpoint4/epoch14 --task intent --do_lowercase --max_seq_length 150 --dump_outputs \
