...
from transformers import BertConfig, BertModel, BertForMaskedLM, T5EncoderModel, T5ForConditionalGeneration
#from space.models.t5_model import T5_model
from typing import Any

class T5GenerationPretrain(torch.nn.Module):
    def __init__(self,
                 model_name_or_path: str):
        super(T5Pretrain, self).__init__()
        self.t5_model = T5ForConditionalGeneration.from_pretrained(model_name_or_path)

    def forward(self, 
                input_ids: torch.tensor,
                labels: torch.tensor):
        outputs = self.t5_model(input_ids, labels=labels)
        return outputs[0]

class T5Pretrain(torch.nn.Module):
    def __init__(self,
                 model_name_or_path: str):
        super(T5Pretrain, self).__init__()
        self.t5_model = T5EncoderModel.from_pretrained(model_name_or_path)
...
class IntentT5Model(torch.nn.Module):
    def __init__(self,
                 model_name_or_path: str,
                 dropout: float,
                 num_intent_labels: int,
                 use_observers: bool = False):
        super(IntentT5Model, self).__init__()
        self.t5_model = T5EncoderModel.from_pretrained(model_name_or_path)

        self.dropout = Dropout(dropout)
        self.num_intent_labels = num_intent_labels
        self.intent_classifier = nn.Linear(self.t5_model.config.hidden_size, num_intent_labels) #self.model.shared.num_embeddingsself.t5_model.config.hidden_size
    ...
    def encode(self,
               ...
               token_type_ids: torch.tensor):# not used
        
        hidden_states = self.t5_model(input_ids=input_ids).hidden_states
                                            #attention_mask=attention_mask,
                                            #token_type_ids=token_type_ids)[0]

        pooled_output =  hidden_states[:,:].mean(dim=1)#, -self.num_observers
    ...
    def forward(self,
        ...
        hidden_states = self.t5_model(input_ids=input_ids,attention_mask = attention_mask).last_hidden_state
                                        #attention_mask=attention_mask,
                                        #token_type_ids=token_type_ids)[0]
...
class SlotT5Model(torch.nn.Module):
    def __init__(self,
                 model_name_or_path: str,
                 dropout: float,
                 num_slot_labels: int):
        super(SlotT5Model, self).__init__()
        self.t5_model = T5EncoderModel.from_pretrained(model_name_or_path)
        self.dropout = Dropout(dropout)
        self.num_slot_labels = num_slot_labels
        self.slot_classifier = nn.Linear(self.t5_model.config.hidden_size, num_slot_labels)

    def encode(self,
               input_ids: torch.tensor,
               attention_mask: torch.tensor):
        hidden_states = self.t5_model(input_ids=input_ids,
...
class JointSlotIntentT5Model(torch.nn.Module):
    def __init__(self,
                 model_name_or_path: str,
                 dropout: float,
                 num_intent_labels = 0,
                 num_slot_labels = 0):
        super(JointSlotIntentT5Model, self).__init__()
        self.t5_model = T5EncoderModel.from_pretrained(model_name_or_path)

        self.dropout = Dropout(dropout)
        self.num_intent_labels = num_intent_labels
        self.num_slot_labels = num_slot_labels
        self.intent_classifier = nn.Linear(self.t5_model.config.hidden_size, num_intent_labels)
        self.slot_classifier = nn.Linear(self.t5_model.config.hidden_size, num_slot_labels)
