...
def main(args):
    ...
    dial_index = {}
    split = "test"
    target_jsons = glob(os.path.join(args.data_dir.replace('2.1', '2.2'), "{}/*json".format(split))) if '2.1' in args.data_dir else glob(os.path.join(args.data_dir, "{}/*json".format(split)))
    for target_json_n in target_jsons:
        ...
        for dial_json in target_json:
            ...
            dial_index[dial_id] = dummy_dial_file_json.index(dummy_dial_json)
    ...
    test_path = os.path.join(args.data_dir, 'test_processed.json')
    if '2.1' in args.data_dir and not os.path.exists(test_path):
        test_lines = open(os.path.join(args.data_dir, 'test.json')).readlines()
        dummy_dial_file_test = copy.deepcopy(dummy_dial_file_json)
        for _idx in range(len(idx_lines)):
            ...
            if '2.1' in args.data_dir:
                dial_idx = dial_index[dial_idx]
            val = json.loads(test_lines[_idx])['state'].strip()
            ...
            if val != "NONE":
                ...
                elif dial_json_n == "test":
                    dummy_dial_file_test[int(dial_idx)]["turns"][int(turn_idx)-1]["frames"][int(frame_idx)]["state"]["slot_values"].update({d_s_name: [val]})
            # NONE token means the slot is non-active. Skip the updating option
            else:
                continue
        json.dump(dummy_dial_file_test, open(test_path, 'w'), indent=2)
    ...
    for _idx in range(len(idx_lines)):
        ...
        if val != "NONE":
            ...
            if dial_json_n == "dialogues_001.json":
                ...
                dummy_dial_file_json[int(dial_idx)]["turns"][int(turn_idx)-1]["frames"][int(frame_idx)]["state"]["slot_values"].update({d_s_name: [val]})
        # NONE token means the slot is non-active. Skip the updating option
        else:
            continue
    ...
    if '2.1' in args.data_dir:
        dummy_out_file = open(os.path.join(args.out_dir, "dummy_out.json"), "w")
        json.dump(dummy_dial_file_json, dummy_out_file, indent=2)
