...
def Rouge_L(GOLD, GENR):
    ...
    for guess, answ in zip(GOLD, GENR):
        if answ not in ['',':',"''''''''''''''''''''''''''''''''''''''''''''''''",'- - - - - - - - - - - - - - - - - - - - - - - -', ': : : : : : : : : : : : : : : : : : : : : : :', '........................','a. a. a. a. a. a. a. a. a. a. a. a. a. a. a. a.','.......................'] and guess!='':
            rouge = rouge_PARAI(guess=guess, answers=[answ])
            result = rouge['rouge-l'] if 'rouge-l' in rouge else None
            if isinstance(result,float):
                rouge_score.append(result)
            #else:
            #    print('NAN')
            #    print(guess)
            #    print(answ)
...
def rouge_PARAI(guess: str, answers):
        """
        ...
        evaluator = rouge.Rouge(
            metrics=['rouge-1', 'rouge-2', 'rouge-l'] #, max_n=2
        )
        scores = []
        for a in answers:
            if (normalize_answer(a)!='') and (normalize_answer(guess)!=''):
                scores.append(evaluator.get_scores(normalize_answer(guess), normalize_answer(a)))
        if scores!=[]:       
            scores_rouge1 = max(score[0]['rouge-1']['r'] for score in scores) # add [0]
            scores_rouge2 = max(score[0]['rouge-2']['r'] for score in scores)
            scores_rougeL = max(score[0]['rouge-l']['r'] for score in scores)
            return {
